...
class ModelRegistry:
    ...
    def resolve_cross_model(self, cross_model_ref: str) -> str:
        ...
        This is a strict lookup - cross_model values in domain configs must
        match actual model names exactly. No aliases or remapping.

        Args:
            cross_model_ref: Reference from graph edge cross_model value

        Returns:
            The same reference if valid, raises ValueError if not found

        Raises:
            ValueError: If cross_model_ref is not a valid model name
        """
        if cross_model_ref in self.models:
            return cross_model_ref

        raise ValueError(
            f"Invalid cross_model reference: '{cross_model_ref}'. "
            f"Must be an exact model name. Available models: {self.list_models()}"
        )
