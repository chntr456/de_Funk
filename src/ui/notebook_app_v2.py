"""
Enhanced Streamlit notebook viewer with vault-style navigation.
...
# Configure page
st.set_page_config(
    page_title="Financial Notebook Vault",
    layout="wide",
...
class NotebookVaultApp:
    ...
    def _render_welcome(self):
        ...
        st.title("📚 Financial Notebook Vault")

        st.markdown("""
        ## Welcome to the Notebook Vault

        Your declarative financial modeling workspace with:

        ### 🗂️ Vault Organization
        - Browse notebooks in a directory tree
        - Click any notebook to open it
        - Open multiple notebooks in tabs

        ### 📝 Edit & View Modes
        - Toggle between rendered view and YAML editor
        - Edit notebook configuration directly
        - Save and reload notebooks on the fly

        ### 🎛️ Interactive Filters
        - Filters appear in the sidebar when a notebook is open
        - Changes update all exhibits in real-time
        - Each notebook has its own filter state
        ...
        **Get Started**: Click on a notebook in the sidebar to begin!
        """)
