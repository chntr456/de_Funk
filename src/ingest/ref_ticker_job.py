...
def run_ref_ticker_to_bronze(cfg_path: str, storage_cfg: str, tickers: list[str], snapshot_dt: str | None = None):
    spark: SparkSession = get_spark("Bronze_Ref_Ticker")
    ...
    facet = RefTickerFacet(spark, tickers=tickers)
    raw_batches = []
    ...
    wrote = sink.write_if_missing("ref_ticker", {"snapshot_dt": snap}, df)
    print(f"[ref_ticker] {snap}: {'WROTE' if wrote else 'SKIPPED (exists)'}")
