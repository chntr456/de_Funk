...
from src.data_pipelines.polygon.facets.fundamentals_ttm_facet import FundamentalsTTMFacet

def run_fundamentals_to_bronze(cfg_path: str, storage_cfg: str, tickers: list[str], snapshot_dt: str | None = None):
    spark: SparkSession = get_spark("Bronze_Fundamentals")
    ...
    facet = FundamentalsTTMFacet(spark, tickers=tickers)
    raw_batches = []
    ...
    wrote = sink.write_if_missing("fundamentals", {"snapshot_dt": snap}, df)
    print(f"[fundamentals] {snap}: {'WROTE' if wrote else 'SKIPPED (exists)'}")
